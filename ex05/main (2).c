...
int main() {
    ...
    tracker = 0;
    while(tracker < 127){
        if(ascii_count[tracker] > top_freq){
            next_char = top_char;
            next_freq = top_freq;
            top_char = tracker-1;
            top_freq = ascii_count[tracker];
        }
        if(ascii_count[tracker] > next_freq && ascii_count[tracker] != top_freq){
            next_char = tracker-1;
            next_freq = ascii_count[tracker];
        }
        tracker++;
    }
